...
if __name__ == '__main__':
    ...
    while True:
        ...
        if Utr.size_receive_buffer() != 0:
            receive_object = receive_esp_report(Utr.receive_data(0))
            if isinstance(receive_object, str):
                # Error
                print(receive_object)
            else:
                # Принят объект
                if isinstance(receive_object, Module):  # report = device or sensor  # (!) ----- converter
                    ...
                    print()
                elif isinstance(receive_object, SystemRegistration):  # report = registration  # (!) ----- converter
...
# def up_id(receive_data):  # (!) ----- MQTT
#     receive_object = receive_esp_report(receive_data)  # (!) ----- converter
#     if isinstance(receive_object, str):
#         # Error
#         print(receive_object)
#     else:
...
#     # Передача данных в функции соответствующие топикам
#     if str(msg.topic) in topics:
#         topics[str(msg.topic)](msg.payload)
#     print('----- ----- -----')
#
#
# if __name__ == '__main__':
